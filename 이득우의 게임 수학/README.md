...
#### 10.1 3차원 공간의 설계
- 3차원 공간을 설계하는 방법은 Z축을 모니터가 바라보는 방향과 모니터 뒤편으로 향하는 뒤쪽 방향 두가지로 나뉘는데, 모니터가 바라보는 방향과 일치하는 체계를 오른손 좌표계, 모니터 뒤편을 향하는 체계를 왼손 좌표계라고 한다.
- 유니티와 언리얼은 둘다 왼손 좌표계를 사용하지만 유니티는 위와 아래 방향을 y축, 언리얼은 z축을 사용한다.

#### 10.2 3차원 공간의 트랜스폼
- 2차원 공간에서 이동 변환을 지원하기 위해 3차원 공간을 사용했듯이, 3차원 공간 역시 이동 변환을 위해 한 차원 더 늘어난 4차원 공간을 사용한다.
- 3차원 공간의 회전 변환을 하기 위해 세 표준기저벡터가 동일한 크기와 직교성을 유지한 상태로 함께 움직여야 한다. 회전 변환으로 달라진 세 표준기저벡터값을 알 수 있다면 이를 열벡터로 꽂아 넣어 회전 변환행혈 R을 만들 수 있다.
	- 하지만 이 방법으로 회전을 지정한다는 것은 매번 3개의 변화된 표준기저벡터 값을 계산해야 하기 때문에 여간 까다로운 일이 아닐 것이다. 3차원 공간의 회전을 표현하는 대표적인 방법인 오일러 각(Euler's angle)에 대해 알아보자.
- 오일러 각
	- 3차원 공간에서 물체가 놓인 방향을 3개의 각을 사용해 표시하는 방법
	- 다음의 3차원 벡터로 표현할 수 있다.
		- (0x, 0y, 0z)
	- 엔진마다 축의 용도가 다르기 때문에, 방향에 따라 요(Yaw), 롤(Roll), 피치(Pitch)로 구분하여 회전을 표현한다. 요, 롤, 피치의 움직임으로 오일러 각을 지정하면, 서로 다른 좌표계의 프로그램간에도 데이터를 쉽게 변환할 수 있다.
#### 10.4 오일러 각의 특징
- 장점
	- 직관적인 인터페이스
	- 적은 용량으로 3차원 공간의 회전 정보 기록
- 단점
	- 짐벌락 현상: 회전에 제약이 생기는 현상, 짐벌락 현상을 방지하고 싶다면 회전의 구현 방식을 바꿔서 로드리게스 회전 공식을 사용하거나 사원수를 사용해야 한다.

### 11장 외적: 3차원 공간의 분석와 응용
#### 11.1 벡터의 외적
- 3차원 벡터의 외적은 x 기호를 사용한다.
	- u = (ux, uy, uz), v = (vx, vy, vz)
	- u x v = (uyvz - vyuz, uzvx - vzux, uxvy - vxuy)
- 연산 결과가 항상 스칼라로 나오는 내적과 다르게 외적의 결과는 언제나 3차원 벡터가 된다.
- 평행성 판별
	- 동일한 벡터끼리 내적하면 벡터 크기를 제곱한 값이 나오는데, 동일한 벡터를 외적하면 항상 영벡터가 나온다.
	- 평행한 벡터와 외적하면 크기와 관계 없이 영벡터가 된다.
	- 외적으로 생성된 벡터의 크기는 두 벡터가 만드는 평행사변형의 넓이로 나타낼 수 있다.
	- 내적으론 직교 여부를 판단 가능
- 법선 벡터
	- 벡터 외적은 두 벡터에 모두 직교하는 벡터를 생성한다.
	- 두 벡터의 외적은 두 벡터가 만드는 평면에 직교하는, 다시 말하면 평면이 향하는 방향에 대한 벡터를 만들어내는 것으로 해석할 수 있다.
- 좌우 방향 판별
	- 물체가 시선 방향에서 왼쪽에 있는 경우: (f x v) · y > 0
	- 물체가 시선 방향에서 오른 쪽에 있는 경우: (f x v) · y < 0
	- 물체가 정확히 시선 방향과 일치하는 경우 (f x v) = 0
	- 내적으론 물체가 앞, 뒤에 있는지 여부를 판단 가능하다.
	- 게임 개발에서 유용하게 사용할 수 있다.

#### 11.3 렌더링 계산량을 줄여주는 백페이스 컬링
- 게임은 실시간으로 빠르게 렌더링을 처리해야 하기 때문에 시선에 감지되지 않는 메시의 뒷면은 그리지 않고 건너뛰는 것이 효과적이다. 이렇게 카메라와 마주보지 않는 뒷면을 생략하는 기법을 백페이스 컬링(Backface culling)이라고 한다.
- 삼각형을 구성하는 정점 버퍼, 인덱스 버퍼에서 점의 순서를 활용하여 외적을 사용해 삼각형이 향하는 방향을 파악할 수 있다.
	- 삼각형이 시계방향일 경우 지면의 뒷편을 향하고, 반시계 방향일 경우 지면의 앞쪽을 향한다.
	- 삼각형이 향하는 방향과 카메라가 마주볼경우 렌더링하고, 마주보지 않을 경우 렌더링 하지 않는 것이 백페이스 컬링의 알고리즘이다.

#### 11.4 오일러 각의 문제를 해결하는 로드리게스 회전 공식
- 오일러 각의 문제(짐벌락, 회전 보간이 어려움)는 임의의 축에 대한 평면의 회전 방식을 사용하면 해결이 가능하다. 이 방식을 축-각 회전(Axis-Angle rotation)이라 부른다.
- 로드리게스 회전 공식을 활용하면 오일러 각으로 구현하기 어려운 임의의 축에 대한 회전 변환을 수행할 수 있다. 하지만 이 방식은 행렬로의 변환이 어렵다보니 지금까지 구축한 렌더링 파이프라인에 연동하기가 까다롭다.
- 따라서 게임 엔진에서는 로드리게스 회전과 동일한 기능을 제공하면서도 간결하고 행렬로 변환이 용이한 사원수를 사용한다.

### 11.5 삼중곱
- 벡터의 외적과 내적을 두 번 연속 사용하는 삼중곱(Triple Product) 연산에 대해 알아보자
- 내적과 외적의 순서에 따라 다음과 같은 경우의 수가 발생한다.
	1. u·(v·w)
	2. u·(v x w)
	3. u x (v·w)
	4. u x (v x w)
	- 이 중 1번 연산에서 괄호 안의 벡터 내적의 결괏값은 스칼라가 되므로 해당 삼중곱은 벡터를 세 번 곱했다고 보기 어렵다.
	- 3번 연산의 경우 벡터와 스칼라는 외적할 수 없으므로 연산이 불가능하다.
	- 따라서 2번과 4번 연산만이 유효한 삼중곱 연산이 된다.
- 스칼라 삼중곱
	- 2번 삼중곱 연산(u·(v x w))을 4번 삼중곱 연산과 구분하기 위해 별도로 스칼라 삼중곱(Scalar triple product)이라고 부른다.
	- 스칼라 삼중곱의 절댓값은 세 벡터(u, v, w)가 만드는 평행육면체(Parallelepiped)의 부피를 의미한다.
	- 스칼라 삼중곱은 세 벡터가 모두 선형 독립의 관계를 가지는 지 판단하는 판별식이다.
		- 삼중곱의 값이 0이나오는 경우는 두 벡터 v, w가 평행이거나, v와 w가 만드는 평면에 벡터 u가 속하는 경우이다.
- 벡터 삼중곱
	- 4번 삼중곱 연산 u x (v x w)에 대해 알아보자.
	- 벡터 삼중곱으로 만들어지는 벡터는 두 벡터 v와 w가 만드는 평면에 속한다.
	- 동일 평면에서 벡터 u와 직교하는 벡터는 (u x v) x u의 벡터 삼중곱을 사용해 구할 수 있다.

### 12장 원근 투영: 화면에 현실감을 부여하는 변환
#### 12.1 원근 투영 변환의 원리
- 우리가 만든 3차원 공간에 투시 원근법의 원리를 적용하기 위해서는, 공간의 모든 점이 한 점을 향해 모이는 형태로 변경해야 한다. 이러한 변환을 원근 투영 변환(Perspective projection transformation)이라고 부른다.
- 원근 투영 변환을 설계하기 위해서는 눈에 보이는 범위를 카메라에도 설정해야 하는데 이를 화각(Field of view)라고 한다.
- 원근 투영 변환은 x,y,z 축이 모두 직교하는 정육면체 형태를 가진 뷰 공간을 카메라의 한점으로 모이는 사각뿔 형태를 가진 공간으로 변환하는 작업이라고 할 수 있다.
- 3차원 공간을 변환한 후에는 공간의 물체를 투영해 2차원의 모니터 평면에 담아내야 한다. 이를 위해서 모든 물체의 상이 맺히는 가상의 평면을 생성해야 하는데, 이를 투영 평면(Projection plane)이라고 부른다.
	- 투영 평면의 위치가 카메라로부터 멀어질수록 투영 평면은 더 커지고, 카메라로부터 투영 평면까지의 거리를 초점 거리(Focal length)라고 한다.
- 투영 평면에 대응하는 정사각형 영역은 2차원 평면의 좌표 시스템을 가지는데 이를 NDC(Normalized device coordinate)라고 부른다. NDC는 가운데 중점을 원점으로 설정한다.
- NDC 좌표를 화면 해상도만큼 늘려주면 최종 스크린 좌표가 완성 되는데, 대부분 해상도가 1:1로 균등하지 않기 때문에 물체가 찌그러지는 현상이 발생한다. 화면의 가로 세로 비를 종횡비(Aspect ratio)라고 한다.
	- 이 문제를 해결하는 방법은 화면의 종횡비를 미리 파악해, NDC영역에서 미리 찌그러트린 후에 펼치는 것이다.
	- 종횡비의 역수만큼 미리 찌그러트린 후 펼치면 된다.

#### 12.2 동차 좌표계
- NDC는 2차원 평면의 좌표지만 모든 점에 사용 가능한 원근 투영 행렬을 사용하도록 계산 과정을 두 단계로 구분하고 사용하는 벡터의 값도 한 차원 높였다. 이렇게 한 차원 높인 벡터를 사용하는 것을 게임 제작에서는 보통 동차 좌표계(Homogenous coordinate system)를 사용한다고 표현한다.
- 사영 공간의 점과 투영된 점의 좌표는 카메라로부터 멀어질수록 원점에 가까워지고, 카메라에 가까워질수록 커지는 반비례 관계를 가진다.
- NDC의 원점은 투시 원근 기법에서 이야기하는 소실점(Vanishing point)에 해당한다.

#### 12.3 깊이 값
- 우리에게 보이는 화면은 2차원 평면이므로 나중에 그린 물체가 앞에 보일 수 밖에 없다. 가장 가까운 물체를 가장 크게 배치했지만, 가장 먼저 그려서 가장 먼 곳에 있는 것처럼 보일 수 있다.
- 물체가 카메라로부터 얼마나 떨어져 있는지에 대한 정보를 기록하여 앞에 있는 물체를 나중에 그리도록 설정할 수 있다. 이 데이터를 깊이(Depth) 값이라고 한다.

#### 12.4 원근 보정 매핑
- 사영 공간에서의 무게중심좌표와 NDC에서의 무게중심좌표가 서로 달라 투영 과정에서 찌그러져 보일 수 있다.
- 이를 바로잡으려면 투영되기 전 사영 공간에서의 무게중심좌표를 사용해야 한다. 따라서 투영 과정을 거꾸로 추적해 NDC에서 구한 무게중심좌표로부터 사영 공간의 무게중심좌표를 계산해야 한다.
- 이 과정을 투영 보정 보간(Perspective correction interpolation)이라고 한다.
- 투영 보정 보간의 수식을 활용해 찌그러진 텍스처를 피는 것을 원근 보정 매핑(Perspective correction mapping)이라고 한다.

#### 12.5 깊이 버퍼
- 같은 거리에 있는 두 게임 오브젝트가 서로 겹쳐 있다면, 물체 단위로 그리는 순서를 조절하는 것으로 원근감 문제를 해결할 수 없다.
- 근본적인 해결 방법은 게임 오브젝트의 단위가 아닌 삼각형의 픽셀 단위로 깊이를 비교하고 가까운 곳에 있는 픽셀만 그리는 것이다.
- 이를 구현하기 위해 화면의 픽셀마다 현재의 값을 보관해주는 별도의 저장 공간이 필요한데, 이를 깊이 버퍼(Depth buffer)라고 한다.
- 최종 픽셀을 찍는 과정에서 현재 깊이 값을 깊이 버퍼에 저장된 값과 비교해 현재 깊이 값이 작은 경우에만 픽셀을 찍도록 로직을 구성하면 그리기 순서 문제를 해결하면서, 물체에 가려져 그리지 않아도 되는 픽셀을 파악해 그리기를 건너뛸 수 있다. 이 작업을 깊이 테스팅(Depth testing)이라고 한다.

#### 12.6 정리
- 3개의 축이 모두 직교하는 정육면체 형태의 유클리드 공간을 고정된 점에서 퍼져나가는 사각뿔 형태의 사영 공간으로 변환하는 원근 투영 행렬을 설계하는 과정
- 원근감이 부여된 상황에서 텍스처를 입힐 때는 원근 투영된 화면에서의 무게중심좌표가 아니라 투영 되기 전의 무게중심좌표를 사용해야 텍스처 매핑이 올바로 적용됨을 확인할 수 있었다.
- 다수의 물체가 겹치는 상황에서 앞에 위치한 픽셀만 렌더링하는 깊이 테스팅 기법을 직접 구현하고 깊이 값이 가지는 비선형적 성질을 확인했다.

### 13장 절두체: 최적화된 3차원 공간
- 게임에서는 실시간으로 빠르게 물체들을 그려내야 하기 때문에 계산량을 조금이라도 줄이는 방법을 적극적으로 도입하고 있다. 가장 효과적인 방법은 시야에 보이는 물체만 그리는 것이다. 이의 대표적인 기법으로는 11장 벡터 외적에서 배운 백페이스 컬링을 들 수 있다. 13장에서는 절두체 컬링을 알아본다.

#### 13.1 절두체 컬링
- 다수의 게임 오브젝트가 넓게 퍼져 있는 경우, 카메라 시야 밖에 위치한 게임 오브젝트까지 모두 그리는 것은 비효율적인 작업이다. 그래서 게임에서는 효과적인 렌더링을 위해 절두체 영역에 속한 게임 오브젝트만 그린다. 절두체의 영역 밖에 있는 게임 오브젝트를 파악하고 걸러내는 기능을 절두체 컬링(Frustum culling)이라고 한다.
- 절두체 컬링은 절두체를 구성하는 6개의 평면에 대해 각각 평면의 방정식을 세우고, 평면의 방정식을 활용해 대상 게임 오브젝트의 위치가 평면의 바깥에 있는지 확인하는 절차로 진행 된다.

#### 13.2 바운딩 볼륨
- 카메라를 움직일 때 절두체 컬링이 화면 가장자리에 있는 게임 오브젝트를 갑자기 사라지거나 튀어나오게 하는 현상을 만들 수 있다.
- 이 현상이 발생하는 이유는 게임 오브젝트의 메시가 차지하는 공간 영역이 있음에도 불구하고 게임 오브젝트의 기준 위치만 가지고 절두체 컬링을 수행했기 때문이다.
- 이 문제를 해결하기 위해서는 게임 오브젝트의 위치를 대상으로 하지 않고, 메시가 차지하는 영역을 감안해 절두체 컬링을 진행해야 한다. 게임 엔진은 메시가 차지하는 영역을 효과적으로 관리하기 위해 구나 박스 같은 원시 도형을 사용하는데, 이러한 원시 도형으로 설정한 공간 데이터를 바운딩 볼륨(Bounding volume)이라고 한다.
- 바운딩 볼륨에서 손쉽게 사용되는 원시 도형은 구다. 구는 두 영역이 서로 겹치는지, 떨어져 있는지를 가장 쉽고 빠르게 파악할 수 있는 도형이기 때문이다. 어떤 점이 구의 외부있는지 파악하는 방법은 중점으로부터의 거리와 반지름을 비교하면 쉽게 파악할 수 있다.
- 박스 영역을 사용해서 좀 더 정교한 절두체 컬링 작업을 수행할 수 있다.
	- 박스 영역을 구성하는 각 축의 최댓값과 최솟값을 설정하고, 메시를 구성하는 점을 순회하면서 각 차원의 값을 비교한다.
	- 현재 박스 영역에 저장된 최댓값보다 크거나 최솟값보다 작다면 박스 영역의 최댓값과 최솟값을 갱신한다.
	- 모든 점에 대해 이 과정을 거치면 기저 축에 평행한 박스 영역이 형성되는데, 기저 축에 정렬되어 있다는 의미로 해당 바운딩 볼륨을 AABB(Axis aligned bounding box)라ㅣ고 부른다.

#### 13.3 삼각형 클리핑
- 삼각형의 한 점이 카메라 뒤에 있을 경우 원점을 중심으로 투영되기 때문에 엉뚱한 삼각형이 그려질 수 있다.
- 평면이 올바르게 보이려면 카메라 뒤쪽에 있는 점을 파악해 점이 거꾸로 뒤집혀 투영되지 않도록 예방해야 한다.
- 문제 해결을 위해 카메라 뒤에 있는 삼각형의 영역을 파악하고, 이 영역을 잘라내야 한다. 이를 삼각형 클리핑(Triangle clipping)이라고 한다.
- 삼각형 클리핑은 월드 공간의 좌표를 사용하지 않고 투영 행렬을 적용한 후의 좌표를 사용한다. 사영 공간에서 진행하는 것이 더 간편하기 때문.